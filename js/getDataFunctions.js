	  ...
	  dataType:		"json"
		...
		$('#ThemenlisteTree').empty()
			.jstree({   'plugins':  [ "themes" ],
				...
				{ 
					getChallenges($('#btn_Aufgabeauswahl').html() == "Alle Aufgaben anzeigen" ? data.node.id.split('_')[1] : ALLEAUFGABENANZEIGEN); 
				}
			);
			...
			responseData.forEach(function(item, idx) {
				...
				$('#ThemenlisteTree').jstree(true).create_node(
					item.Parent == null ? '#' : 'T_' + item.Parent,
					{
						"id":				'T_' + item.Id,
						"text":			item.Thematext,
...
function xxxxAddTreeviewSubItems(Themes, item, ul_parent) {
	...
	var Subitems = $.grep(Themes, function(theme, i) { return (theme['Level'] == parseInt(item['Level']) + 1 && theme['parent'] == item['Thematext']); });
	...
	$.ajax({
		...
		$( "#ul_Aufgabenliste" ).selectable({
			selected: function( event, ui ) {
				...
				if($('input[name="Spielinteraktion"]:checked').val() == "Lichess") {
					showChallengelichess(ui.selected.id)
				} else {
