...
function offerEngineVariationsStart(V_Zuege, OmittedMove) {
    ...
    EngineMovesToStack(OmittedMove, V_Zuege);

    processChallengeStartOfferAnswer = new $.Deferred(); // Hier, da die Funktion sich rekursiv aufruft
    ...
    processChallengeStartOffer(Zugtext(OmittedMove.ZugKurz)).then( function(decision) {
        console.log('processChallengeStartOffer decision: ', decision);
...
function offerEngineVariationsEnde() {
    ...
    processEngineEndeOfferAnswer = new $.Deferred();

    processEngineEndeOffer(EndMove).then( function(decision) {
        console.log('processEngineEndeOffer decision: ', decision);
...
function EngineMovesToStack(Hauptzug, Variantenzuege) {
    ...
    Stellungsdaten.ZugStack.push( { 
        ...
        ChildMove:  "",
        ZeigeZug:   true // Enginezüge werden immer gezeigt
    });
