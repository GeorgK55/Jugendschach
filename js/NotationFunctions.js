...
function NewTreeNode(TreeContainer, Mode, Situation, Zug, TooltipFlag) {
    ...
    var NewTreeNodeId;
		...
		$("span[id^='M_']").removeClass("currentmovemarker");
		$("span[id^='M_']").addClass("formermovemarker");
    ...
    if (Mode == 'sign') { 
        ...
        if(Zug.ZugFarbe == WEISSAMZUG) {
            htmlNodeText_w  = "<span class='movewhite currentmovemarker' id='" + Situation.CurMoveId + WhitePostfix
                            ...
                            + "' onclick='jumpToPosition(\"Brett_SpieleAufgabe\", \"" 
...
function removeNotationMarker(TreeContainer) {
	...
	var nodesarray = $('#' + TreeContainer).jstree(true).get_json('#', { flat: true })

	nodesarray.forEach(function(item, idx) {
		if(item.text.includes("currentmovemarker")) {
			console.log(item.id);

			var changenode = $('#' + TreeContainer).jstree(true).get_node(item.id); 

			var changetext = changenode.text.replace(' currentmovemarker', '');
			$('#' + TreeContainer).jstree().rename_node(changenode, changetext);
...
function addNotationMarker(TreeContainer, Farbe, KnotenId) {
	...
	var changenode = $('#' + TreeContainer).jstree(true).get_node(KnotenId); 
	var changetext = "";
